...
while True:
    ...
    while target_stack: # target_stack이 빌때까지
        ...
        for holes in HOLES:
            if where(balls[0], target_now) == 1:
                if holes[0] > target_now[0] and holes[1] > target_now[1]:
                    Hole = holes
                    break
            elif where(balls[0], target_now) == 2:
                if holes[0] < target_now[0] and holes[1] > target_now[1]:
                    Hole = holes
                    break
            elif where(balls[0], target_now) == 3:
                if holes[0] < target_now[0] and holes[1] < target_now[1]:
                    Hole = holes
                    break
            elif where(balls[0], target_now) == 4:
                if holes[0] > target_now[0] and holes[1] < target_now[1]:
                    Hole = holes
                    break
        """
        ...
        real_target = []
        power_width = abs(float(Hole[0]) - float(target_now[0]))
        power_height = abs(float(Hole[1]) - float(target_now[1]))
        seta = math.atan((abs(float(Hole[1]) - float(target_now[1]))) / (abs(float(Hole[0]) - float(target_now[0]))))
        if where(target_now, Hole) == 1:
            real_target = [float(target_now[0]) - 5.73 * math.cos(seta), float(target_now[1]) - 5.73 * math.sin(seta)]
        ...
        width = abs(float(real_target[0]) - float(whiteBall_x))
        height = abs(float(real_target[1]) - float(whiteBall_y))
        ...
        distance1 = float(math.sqrt(width**2 + height**2))
        distance2 = float(math.sqrt(power_width ** 2 + power_height ** 2))
        distance = max(distance1, distance2)
        power = distance * 0.45
        """
        if distance2 <= 10:   # 공이 많이 가까울 때
            power = distance * 0.4
        elif 10 < distance2 <= 25:   # 공이 적당히 가까울 때
            power = distance * 0.45
        else:
